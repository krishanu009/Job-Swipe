...
const Swipe = () => {
  ...
  const rejectResume = () => {
    ...
    //logic for api call to reject resume
      ...
      console.log("resume to reject",currentResume);

    let newResumeData = resumeData.filter(item => 
      item.id !== currentResume.id
    );
    setResumeData(newResumeData);
    if(newResumeData.length)
    setCurrentResume(newResumeData[0]);
    else setCurrentResume({
      id:"",
      name: "",
      tags: "",
      summary:
  ...
  const selectResume = () => {
    ...
    //logic for api call to select resume
      ...
      console.log("resume to select",currentResume);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <img src={require("../assets/rejected.png")} alt="Checked" />
        </div>
      </div>
     {!resumeData.length? (<div></div>):(<div className=" w-[1/3] flex justify-center items-center h-screen">
